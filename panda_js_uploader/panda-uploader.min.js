...
a.cleanUp=function(b){try{if(this.movieElement&&typeof b.CallFunction==="unknown"){this.debug("Removing Flash functions hooks (this should only run in IE and should prevent memory leaks)");for(var c in b)try{if(typeof b[c]==="function")b[c]=null}catch(d){}}}catch(e){}window.__flash__removeCallback=function(f,g){try{if(f)f[g]=null}catch(l){}}};a.fileDialogStart=function(){this.queueEvent("file_dialog_start_handler")};
...
(function(b){var c=["swfupload_loaded_handler","file_queued_handler","file_queue_error_handler","file_dialog_start_handler","file_dialog_complete_handler","upload_start_handler","upload_progress_handler","upload_error_handler","upload_success_handler","upload_complete_handler","queue_complete_handler"],d=[];b.fn.swfupload=function(){var e=b.makeArray(arguments);return this.each(function(){var f;if(e.length==1&&typeof e[0]=="object"){f=b(this).data("__swfu");if(!f){var g=e[0],l=b(this);f=[];b.merge(f,
c);b.merge(f,d);b.each(f,function(q,m){var o=m.replace(/_handler$/,"").replace(/_([a-z])/g,function(h,k){return k.toUpperCase()});g[m]=function(){var h=b.Event(o);l.trigger(h,b.makeArray(arguments));return!h.isDefaultPrevented()}});b(this).data("__swfu",new SWFUpload(g))}}else if(e.length>0&&typeof e[0]=="string"){var n=e.shift();(f=b(this).data("__swfu"))&&f[n]&&f[n].apply(f,e)}})};b.swfupload={additionalHandlers:function(){if(arguments.length===0)return d.slice();else b(arguments).each(function(e,
f){b.merge(d,b.makeArray(f))})},defaultHandlers:function(){return c.slice()},getInstance:function(e){return b(e).data("__swfu")}}})(jQuery);
(function(){function b(c){this.options=c}jQuery.fn.pandaUploader=function(c,d,e){function f(){k.closest("form").submit(l)}function g(i,j){i=$("#"+d.upload_filename_id);i.size()!=0&&i.val(j.name)}function l(){h.swfupload("startUpload");return false}function n(i,j){d.progress_handler&&d.progress_handler.start(j)}function q(i,j,p,r){try{d.progress_handler&&d.progress_handler.setProgress(j,p,r)}catch(s){}}function m(i,j,p){k.val(eval("("+p+")").id)}function o(){var i=k.closest("form")[0];document.createElement("FORM").submit.apply(i)}
if(c===undefined){alert("There was an error setting up the upload form. (The upload parameters were not specified).");return false}d=d===undefined?{}:d;e=e===undefined?{}:e;if(d.upload_button_id===undefined){alert("You have to specify the button id");return false}d=jQuery.extend({upload_filename_id:null,upload_progress_id:null,api_url:"http://staging.pandastream.com/v2/videos.json",progress_handler:null},d);if(!d.progress_handler)d.progress_handler=new b(d);var h=this.swfupload(jQuery.extend({upload_url:d.api_url,
file_size_limit:0,file_types:"*.*",file_types_description:"All Files",file_upload_limit:0,flash_url:"/panda_js_uploader/swfupload-jquery/swfupload/swfupload.swf",button_image_url:"/panda_js_uploader/swfupload-jquery/swfupload/XPButtonUploadText_61x22.png",button_width:61,button_height:22,button_placeholder_id:d.upload_button_id,post_params:c,file_post_name:"file",debug:true},e)),k=this;h.bind("swfuploadLoaded",f);h.bind("fileQueued",g);h.bind("uploadStart",n);h.bind("uploadProgress",q);h.bind("uploadSuccess",
m);h.bind("uploadComplete",o)};b.prototype={start:function(c){var d=jQuery("#"+this.options.upload_progress_id);if(d.size()!=0){d.append('<div class="progress-inside"></div>');this.progress=d.find(".progress-inside");this.setProgress(c,0,c.size)}},setProgress:function(c,d,e){if(this.progress){c=Math.ceil(d/e*100);$(this.progress).css("width",c+"%")}}}})();
